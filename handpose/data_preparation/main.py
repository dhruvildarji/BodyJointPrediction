...
def create_aria_calib(args):
    ...
    vrs_file_path = f"/mnt/d/git/dataset/handPose/data/takes"
    dirs = os.listdir(vrs_file_path)
    for dir_name in dirs:
        vrs_folder = Path(f"{vrs_file_path}/{dir_name}")
        vrs_path = vrs_folder.glob("*.vrs")
        vrs_path = [str(file) for file in vrs_path]
        # if len(list(vrs_path)) == 0:
        # # if not os.path.exists(vrs_path):
        #     print(
        #         f"[Warning] No take vrs found at {vrs_path}. Skipped take {dir_name}."
        #     )
        #     continue
        print(vrs_path)
        if len(vrs_path) == 0:
        ...
        if not os.path.exists(vrs_path):
            ...
            continue
        output_path = os.path.join(aria_calib_json_output_dir, f"{dir_name}.json")
        print(output_path)
       
        extract_aria_calib_to_json(vrs_path, output_path)
        # 2. Overwrite f, cx, cy parameter from JSON file
        if not Path(output_path).parent.exists():
            os.system(f'mkdir -p {Path(output_path).parent}')
