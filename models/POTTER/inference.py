...
def main(args):
    ...
    valid_dataset = ego4dDataset(
        args, cfg, split=args.split, transform=transform)
    # Dataloader
    ...
    with torch.no_grad():
        ...
        for input, _, _, meta in valid_loader:
            # Pose 3D prediction
    ...
    pred_output_path = os.path.join(args.output_dir, f"ego_pose_pred_anno_{args.split}.json")
