                ...
                for frame_number in tqdm(take_anno.keys(), total=len(take_anno.keys())):
                    ...
                    if not os.path.exists(curr_undist_img_path):
                        ...
                        curr_dist_image = np.array(
                            Image.open(curr_dist_img_path))
                        curr_dist_image = (
                            cv2.rotate(curr_dist_image,
        ...
        for split in args.splits:
            ...
            for i, (take_uid, take_anno) in enumerate(gt_anno.items()):
                ...
                if not os.path.exists(curr_take_video_path):
                    ...
                    continue
                curr_take_img_output_path = os.path.join(
                    img_output_root, take_name)
                ...
                for frame_number in tqdm(take_anno.keys(), total=len(take_anno.keys())):
                    ...
                    if not os.path.exists(out_path):
                        ...
                        frame = frame if args.portrait_view else np.rot90(
                            frame)
...
def create_gt_anno(args):
    """
    Creates ground truth annotation file for train, val and test split.
    """
...
def create_aria_calib(args):
    ...
    aria_calib_json_output_dir = os.path.join(
    ...
    for take_name, _ in tqdm(take_to_uid.items()):
        ...
        assert len(
            take) == 1, f"Take: {take_name} can't be found in takes.json"
        take = take[0]
        ...
        output_path = os.path.join(
            aria_calib_json_output_dir, f"{take_name}.json")
        ...
        aria_cam_calib = [
            c for c in all_cam_calib if c["Label"] == "camera-rgb"][0]
