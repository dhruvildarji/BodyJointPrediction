...
class ego_pose_anno_loader:
    ...
    def __init__(self, args, split, anno_type):
        ...
        # Dimension of undistorted aria image [H, W]
        ...
        self.aria_calib_dir = os.path.join(
            args.gt_output_dir, "aria_calib_json")
        self.takes = json.load(
            open(os.path.join(self.dataset_root, "takes.json")))
    ...
    def load_raw_data(self):
        ...
        # Find all annotation takes from local directory by splits
        ...
        comm_take_w_cam_pose = list(
            set(common_take_uid) & set(available_cam_pose_uid))
        print(
        ...
        for curr_take_uid in comm_take_w_cam_pose:
            ...
            print(f"Processing take: {curr_take_name}")
            print(f"Annotation path: {curr_take_anno_path}")
            print(f"Camera pose path: {curr_take_cam_pose_path}")

            # Load in annotation JSON and image directory
            if not os.path.exists(curr_take_anno_path):
                print(f"Annotation file not found: {curr_take_anno_path}")
                continue
            if not os.path.exists(curr_take_cam_pose_path):
                print(f"Camera pose file not found: {curr_take_cam_pose_path}")
                continue
            ...
            try:
                ...
                print(
                    f"Failed to open camera pose file {curr_take_cam_pose_path}: {e}")
                continue
        ...
        for frame_idx, curr_frame_anno in anno.items():
            ...
            for hand_idx, hand_name in enumerate(HAND_ORDER):
                ...
                one_hand_3d_kpts_cam = world_to_cam(
                    one_hand_3d_kpts_world, curr_extri)
                # Camera to image plane
                one_hand_proj_2d_kpts = cam_to_img(
                    one_hand_3d_kpts_cam, curr_intri)
    ...
    def load_aria_calib(self, curr_take_name):
        ...
        dst_cam_calib = calibration.get_linear_camera_calibration(
            512, 512, 150)
    ...
    def one_hand_kpts_valid_check(self, kpts, aria_mask):
        ...
        invis_flag = (
            aria_mask[new_kpts[:, 1].astype(
                np.int64), new_kpts[:, 0].astype(np.int64)]
            == 0
        )
        # 4. Get valid flag
